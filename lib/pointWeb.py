...
class PointWeb(Graph):
    ...
    def __init2__(self, distIm, points, maxDist, fiberW):
        ...
        for p0 in self:
            ...
            nearby = KDT.query_ball_point(p0, maxDist)
...
#                 print("\t@@", n.e, n.e in self)
    ...
    def __initSlow__(self, distIm, points, maxDist, fiberW):
        # im0 is a binary image where all points which should be connected are connected
        # by a straight line of white pixels.
        # for each point, look in a circle until you hit another point or reach the maximum
        # link distance. 
...
#                 print("\t@@", n.e, n.e in self)
    ...
    def __init__(self, imForSize, points, maxDist, fiberW):
        # im0 is a binary image where all points which should be connected are connected
        # by a straight line of white pixels.
        # for each point, look in a circle until you hit another point or reach the maximum
        # link distance. 
#         radiusList = np.arange(1.0 , maxDist, 0.4)
#         thetaList0 = np.arange(0 , 2*pi, pi/60)
        radiusList, thetaMatrix0 = Graph.getradialCircleArray(maxDist)
...
#             print(tempDict[p0].e.pnts[0], tempDict[p0].e.pnts[-1], pAvg)
            ...
            if i%100 == 0:
                print(i, len(points), "building fiber graph")
...
#             i+=1
            ...
            for iR in range(0, len(thetaMatrix)):
                ...
                for iT in range(0, len(thetaMatrix[0])):
                    t = thetaMatrix[iR][iT]
                    if t == -1: # end of values at that radius
                        break
                    if t == -2: # theta value is no longer valid
                        continue
